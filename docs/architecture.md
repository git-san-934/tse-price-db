...
### 実運用で判明したレート制限の影響とラウンド制リトライ
- 初回実装(並列数10・リクエスト間隔0.2秒)で実行したところ、実行時間が約2分と
  極端に短く、約4,449銘柄中442銘柄(約1割)しか発行済株式数を取得できなかった。
  短時間で終わっていることから、Yahoo Finance側が大量の個別リクエストを検知し、
  多くのリクエストが待たされることなく失敗していたと考えられる。
- 対策として、(1)並列数を3・リクエスト間隔を1.0秒に緩め1銘柄あたりの負荷を下げる、
  (2)失敗した銘柄だけを対象に、ラウンド間に90秒のクールダウンを挟みながら最大4
  ラウンド再試行する、という2点を導入した。ジョブのタイムアウトも120分から
  180分に延長した(複数ラウンドの再試行時間を確保するため)。
