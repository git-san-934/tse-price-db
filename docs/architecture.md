...
- この設計により、`data/prices.db`は「無限に蓄積される長期データベース」ではなく
  「直近約200日分のローリングウィンドウ」になる。

## 5年分チャート(週次データによる長期履歴)
- 銘柄詳細のチャートで5年分を見たいという要望に対し、日次DBの保持期間をそのまま
  延ばすとGitHubの100MB制限を再び超えてしまう(全銘柄×5年分の日次データは
  400MB超と試算)。
- そこで、**週次(1週間に1点、終値・出来高のみ)** の別データベース
  `data/prices_weekly.db` を新設した。日次DB(`data/prices.db`)とはファイルを
  分離し、サイズ予算を独立させている。
- `scripts/fetch_prices.py`の`sync_weekly_prices()`が`yf.download(period="5y",
  interval="1wk")`で全銘柄の週足を取得し、`db_common.prune_old_weekly_prices()`
  (`WEEKLY_RETENTION_DAYS`既定1825日、DELETE後にVACUUM)で5年より古い行を削除する。
  概算では全銘柄(約4,449)×約260週分で50〜60MB程度に収まる。
- 週次のため移動平均線(MA25/MA75)は保持・表示しない。5年スパンでは週次の終値
  折れ線だけで十分に傾向を把握できるという判断。
